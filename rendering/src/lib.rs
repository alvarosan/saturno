...
mod tests {
    ...
    fn render_book_cover() {
        ...
        let mut actors: Vec<Box<dyn RayTraceable>> =
        ...
        for a in -max..max {
            for b in -max..max {
                ...
                let center = arr1(&[a as f64 + 0.9 * rng.gen::<f64>(),
                    0.2, b as f64 + 0.9 * rng.gen::<f64>() as f64, 1.0]);
                let radius = 0.2;
